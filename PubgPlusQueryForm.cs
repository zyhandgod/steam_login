...
namespace SteamLoginLite
{
    internal sealed class PubgPlusQueryForm : Form
    {
        ...
        public PubgPlusQueryForm(IEnumerable<string> gameIds)
        {
            ...
            var header = new Panel { Dock = DockStyle.Top, Height = 72, BackColor = Color.White, Padding = new Padding(18, 8, 18, 8) };
            ...
            _instruction.Height = 26;
            _instruction.Text = "已自动填写查询 ID，请在网页中点击“查询”；如果出现验证码，请正常完成验证。";
        ...
        private async System.Threading.Tasks.Task InitializeAsync()
        {
            ...
            {
                MessageBox.Show("无法启动等级查询窗口。请确认 Microsoft Edge WebView2 Runtime 已安装。\r\n\r\n" + ex.Message, "WebView2 不可用", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Close();
            }
        }
        ...
        private async void OnWebResourceResponseReceived(object sender, CoreWebView2WebResourceResponseReceivedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    var parsed = ParseResult(await reader.ReadToEndAsync(), CurrentId);
                    if (parsed == null) return;
                    BeginInvoke(new Action(() => AcceptResult(parsed)));
                }
            }
            catch { }
        ...
        private static bool IsMatchingRequest(string url, string gameId)
        {
            ...
            {
                var uri = new Uri(url);
                if (!uri.Scheme.Equals("https", StringComparison.OrdinalIgnoreCase) || !uri.Host.Equals("apiv1.pubg.plus", StringComparison.OrdinalIgnoreCase) || uri.AbsolutePath != "/steam/player/info") return false;
                return string.Equals(HttpUtility.ParseQueryString(uri.Query)["player_id"], gameId, StringComparison.OrdinalIgnoreCase);
            }
            catch { return false; }
        }

        private PubgPlusResult ParseResult(string body, string expectedId)
        {
            try
            {
                var root = _json.DeserializeObject(body) as IDictionary<string, object>;
                if (root == null || !root.ContainsKey("code") || Convert.ToInt32(root["code"]) != 0) return null;
                var data = root["data"] as IDictionary<string, object>;
                var player = data?["player"] as IDictionary<string, object>;
                if (data == null || player == null || Convert.ToString(data["type"]) != "ban_info") return null;
                var name = Convert.ToString(player["name"]);
                var shard = Convert.ToString(player["shardId"]);
                if (!string.Equals(name, expectedId, StringComparison.OrdinalIgnoreCase) || !string.Equals(shard, "steam", StringComparison.OrdinalIgnoreCase)) return null;
                var raw = Convert.ToString(player["ban"]);
                if (string.IsNullOrWhiteSpace(raw) || !player.ContainsKey("level")) return null;
                var level = Convert.ToInt32(player["level"]);
                if (level < 0) return null;
                return new PubgPlusResult { GameId = expectedId, RawStatus = raw, Status = BanStatusNormalizer.Normalize(raw), Level = level };
            }
            catch { return null; }
        }
        ...
        private void AcceptResult(PubgPlusResult result)
        {
            ...
            {
                MessageBox.Show("所选账号已查询完成。", "查询完成", MessageBoxButtons.OK, MessageBoxIcon.Information);
